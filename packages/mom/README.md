...
```bash
npm install @propershare/maat-mom
```
...
- run interactive coding agent session: `npx @propershare/maat-coding-agent`
- enter `/login` command
...
Update mom anytime with `npm install -g @propershare/maat-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
