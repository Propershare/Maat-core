...
```typescript
import { type AgentSessionEvent } from "@propershare/maat-coding-agent";
...
import type { MomCustomTool, ToolAPI } from "@propershare/maat-mom";
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@propershare/maat-ai` and `@propershare/maat-mom` are aliased to the running mom bundle.
